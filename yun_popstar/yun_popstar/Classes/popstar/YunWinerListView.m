...
//  yun_popstar
//
//  Created by dangfm on 2020/9/16.
//  Copyright © 2020 fangyun. All rights reserved.
//

#import "YunWinerListView.h"

@implementation YunWinerListView

// 跟上面的方法实现有一点不同
+ (instancetype)sharedSingleton {
    static YunWinerListView *_sharedSingleton = nil;
...
-(void)initViews{
    ...
    UIImage *headertitle = [UIImage imageNamed:@"leaderboard-header-title"];
    hh = 20;
    ...
    UIImage *gkicon = [UIImage imageNamed:@"leaderboard-title-gqph"];
    ...
    UIImage *jbicon = [UIImage imageNamed:@"leaderboard-title-jbph"];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return kYunWinerListCellHeight;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    YunWinerListCell *cell = [[YunWinerListCell alloc] initWithStyle:UITableViewCellStyleDefault
                                  reuseIdentifier:cellIndentifier];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    
    [cell setContentWithDic:_datas[indexPath.row] order:(int)indexPath.row type:_type];
...
-(void)getHttpLeaderList{
    WEAKSELF
    _page ++;
    [http sendPostRequestWithParams:@{@"page":@(_page).stringValue,@"type":@(_type).stringValue} api:kAPI_LeaderList start:^{
